...
void read_display_data(){
	...
	sht35_read_temphumi(&temp,&humi);//加个判断语句,如果crc校验未过，就不参与下面的计算了
	ftemp=temp/(10.0);
	fhumi=humi/(10.0);
	PRINT("temp=%.1f ℃,humi=%.1f ％RH \r\n",ftemp,fhumi );
	PRINT("\r\n");
	OLED_ShowFNum(34,16,ftemp,16,1);
	OLED_ShowFNum(34,32,fhumi,16,1);
	OLED_Refresh();

	modbus_ireg_update(MB_IREG_ADDR_TEMPERATURE,temp);
	modbus_ireg_update(MB_IREG_ADDR_HUMIDITY,humi);
}


//void sht35_run(){
//	 modbus_ireg_update(MB_IREG_ADDR_TEMPERATURE, );
//
//}
